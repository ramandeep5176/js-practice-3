...
async function displayColor() {
  try {
    const first = await addColor(500, heading1, "red");
    await addColor(500, heading2, "green");
    await addColor(500, heading3, "red");
    console.log(first);
  } catch (error) {
    console.log(error);
  }
...
function addColor(time, element, color) {
  return new Promise((resolve, reject) => {
    ...
    } else reject(new Error(`there's no element ${element}`));
  });
}
